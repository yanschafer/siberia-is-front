...
import StockSearchFilterDto, {
  StockSearchDto,
} from "@/api/modules/stock/dto/stock-search-filter.dto";
...
export default class StockModel extends ApiCrudModelUtil<
  StockInputDto,
  StockDto,
  StockUpdateDto
> {
  ...
  ): Promise<ApiResponseDto<StockListItemDto[]>> {
    return this.authorizedRequest(
      new ApiRequestDto(
        "/all",
        "POST",
        new StockSearchFilterDto(stockSearchDto),
      ),
    );
  }
  public async getInputList(): Promise<ApiResponseDto<StockListItemDto[]>> {
    return this.authorizedRequest(new ApiRequestDto("/all/input", "GET"));
  }
}

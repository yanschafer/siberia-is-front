...
export default class ApiResponseDto <T> {
  ...
  static networkError(): ApiResponseDto<any> {
    return new ApiResponseDto(false, null, new ApiErrorDto(
      500, "Server unavailable", "Server unavailable"
    ))
  }
  ...
  static buildFromError(error: AxiosError): ApiResponseDto<any> {
    if (error.response) {
      if (error.response.data.httpStatusCode) {
        return new ApiResponseDto(false, null, error.response.data as ApiErrorDto)
      } else {
        return new ApiResponseDto(false, null, new ApiErrorDto(error.response.status, error.response.statusText, error.response.statusText))
      }
    }
    else {
      return new ApiResponseDto(false, null, new ApiErrorDto(500, "Server unavailable", "Server unavailable"))
    }
  }
}